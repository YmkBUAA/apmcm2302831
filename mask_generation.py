...
def mask():
    levels = np.zeros([1, len(os.listdir(r'D:\code\apmcm\Attachment\Attachment\Attachment1'))])
    for idx, file in enumerate(os.listdir(r'D:\code\apmcm\labels')):
        ...
        for key in point.keys():
            ...
            level = process(tmp_img)
            # print(level)
            level_image.append(level)
            '''
            output_img = img * mask
            ...
            '''
        #print(int(file[:-4]))
        levels[:, int(file[:-4])-1] = (sum(level_image)/len(level_image))
...
# plt.plot(x, levels, label='maturity', color='blue', linestyle='-', linewidth=2, marker='o', markersize=5)
plt.hist2d(x, levels, bins=200, density=False)
plt.title("apple maturity")
